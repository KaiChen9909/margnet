...
def prepare_df(df, dataset, domain):
    num_idx = []
    for i in range(df.shape[1]):
        if df.columns[i].split('_')[0] == 'num' and domain[df.columns[i]] == 1:
            num_idx.append(i)
...
def rap_main(args, df, domain, rho, parent_dir, **kwargs):
    ...
    if args.dataset not in ['gauss10', 'gauss30', 'gauss50']:
        domain = prepare_domain(domain)
    # domain = prepare_domain(domain)

    df, num_encoder, num_idx  = prepare_df(df, args.dataset, domain)
