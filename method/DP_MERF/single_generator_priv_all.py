...
def merf_main(
    ...
    if dataset in homogeneous_datasets:
        ...
        print('the data is heterogeneous')
        model_init = {
    ...
    training_loss_per_epoch = pd.DataFrame(columns=['epo', 'loss'])
    # scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=how_many_iter*how_many_epochs, eta_min=1e-4)
    ...
    for epoch in range(how_many_epochs):  # loop over the dataset multiple times
        ...
        training_loss_per_epoch.loc[len(training_loss_per_epoch)] = [epoch, running_loss]
        if epoch % 50 == 0:
    ...
    training_loss_per_epoch.to_csv(os.path.join(parent_dir, 'loss.csv'))
    ...
    generator = merf_generator(model, model_init, weights, num_encoder, num_numerical_inputs, domain['X_cat'], resample, input_to_model, label_input)
    return {"merf_generator": generator}



def merf_ablation_main(
        args,
        df, 
        domain,
        rho, 
    ...
    dict = merf_main(
        ...
        seed_number = 0,
        is_priv_arg = False,
        preprocesser = kwargs.get('preprocesser', None)
    )
    return dict