...
def sample_multicol_categorical(output_categorical, cat_domain):
    batch_size = output_categorical.shape[0]
    
    samples = []
    start = 0
    for dim in cat_domain:
        probs = output_categorical[:, start:start+dim]
        # sampled_idx = np.array([np.random.choice(dim, p=row) for row in probs])
        sampled_idx = np.array([np.random.choice(dim, p=row / row.sum()) for row in probs])
        
        one_hot = np.zeros((batch_size, dim), dtype=np.float32)
        one_hot[np.arange(batch_size), sampled_idx] = 1
...
class merf_generator():
    def __init__(self, model, model_init, weights, num_encoder, num_numerical_inputs, cat_domain, resample=False, input_to_model=None, label_input=None):
        self.model = model 
    ...
    def sample(self, n, preprocessor, parent_dir, device):
        if not self.resample:
            ...
            if output_categorical.shape[1] > 0:
                output_categorical = sample_multicol_categorical(output_categorical, self.cat_domain)
            ...
            outputs = self.model(self.input_to_model) 
            indices = torch.randint(low=0, high=outputs.shape[0], size=(n,))
            outputs = outputs[indices].cpu().detach().numpy()
            ...
            label_input = self.label_input[indices].cpu().detach().numpy() 
