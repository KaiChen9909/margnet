...
class Dataset:
    ...
    def update_pretrain_data(
            self, 
            df_new: dict
    ):
        if df_new['X_num'] is not None:
            self.X_num['pretrain'] = self.num_transform.transform(df_new['X_num'])
        if df_new['X_cat'] is not None:
            self.X_cat['pretrain'] = self.cat_transform.transform(df_new['X_cat'])
        self.y['pretrain'] = df_new['y']
...
def normalize(
    ...
    elif normalization == 'quantile': # not applicable under DP training (privacy leakage risk)
...
def make_dataset_from_df(
    ...
    if df_pub is not None:
        D = Dataset(
            {'pretrain': df_pub['X_num'], 'train': df['X_num']} if df['X_num'] is not None else None,
            {'pretrain': df_pub['X_cat'], 'train': df['X_cat']} if df['X_cat'] is not None else None,
            {'pretrain': df_pub['y'], 'train': df['y']},
            y_info = {},
            task_type = TaskType(task_type),
            n_classes = y_num_classes
        )
