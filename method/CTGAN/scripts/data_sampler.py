...
class DataSampler(object):
    ...
    def __init__(self, data, output_info, log_frequency, rho=0):
        self._data = data
        ...
        if rho == 0 or n_discrete_columns == 0:
            rho_per_col = 0
        else:
            rho_per_col = rho/n_discrete_columns
        ...
        for column_info in output_info:
            if is_discrete_column(column_info):
                ...
                if rho_per_col > 0:
                    category_freq += np.random.normal(loc=0.0, scale=1/np.sqrt(2*rho_per_col), size=category_freq.shape)
                    category_freq = np.clip(category_freq, a_min=0, a_max=None)
